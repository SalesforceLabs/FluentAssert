...
<#list dataTypes as dataType>
  <@com.apexClass className="${classPrefix}${dataType.native}IsBeforeTest" path="/classes/${dataType.native?lower_case}/"/>
@IsTest
public class ${classPrefix}${dataType.native}IsBeforeTest {
    @IsTest
    static void testPositiveScenarios() {
        ${dataType.native} actual = ${dataType.validValue};
        FluentAssert.that(actual).isBefore(actual${dataType.deltaTemplate?replace("DELTA", "1")});
    }
    ...
    private static void failureScenario(${dataType.native} actual, ${dataType.native} expected) {
        try {
            FluentAssert.that(actual).isBefore(expected);
