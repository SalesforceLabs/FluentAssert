...
<#list dataTypes as dataType>
  <@com.apexClass className="${dataType.native}IsBeforeTest" path="/classes/${dataType.native?lower_case}/"/>
@IsTest
public class ${dataType.native}IsBeforeTest {
    @IsTest
    static void testPositiveScenarios() {
        ${dataType.native} actual = ${dataType.validValue};
        Assert.that(actual).isBefore(actual${dataType.deltaTemplate?replace("DELTA", "1")});
    }
    ...
    private static void failureScenario(${dataType.native} actual, ${dataType.native} expected) {
        try {
            Assert.that(actual).isBefore(expected);
