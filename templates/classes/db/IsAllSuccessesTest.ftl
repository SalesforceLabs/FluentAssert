...
<#list databaseResultTypes as type>
  <@com.apexClass className="List${type.name}IsAllSuccessesTest" path="/classes/db/"/>
@IsTest
public class List${type.name}IsAllSuccessesTest {
    @IsTest
    static void testPositiveScenarios() {
        Assert.that(new List<Database.${type.name}>{${type.successInstance}, ${type.successInstance}})
              .isAllSuccesses();
    }
    ...
    static void testFailureScenarios() {
        ...
        failureScenario(new List<Database.${type.name}>{${type.failureInstance}, ${type.failureInstance}});
    }
    ...
    static void testValidations() {
        try {
            Assert.that((List<Database.${type.name}>) null).isAllSuccesses();
            System.assert(false, 'No assert exception thrown');
        ...
        try {
            Assert.that(new List<Database.${type.name}>()).isAllSuccesses();
            System.assert(false, 'No assert exception thrown');
